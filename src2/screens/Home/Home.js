...
const Home = () => {
  ...
  const dataFetchAPI = async () => {
    ...
    //   setDataAdd(data.slice(item,item + 6));
    //   setItem(item+6);
    //   setDataSource([...dataSource, ...dataAdd]); // Append new data to existing dataSource
    //   if(item == 36)
    //     {
    //       setPage(page => page + 1); // Increment page number for next pagination
    //    // data = await fetchAPI();
    //       setData(await fetchAPI());
    //       setItem(0);
    //     }
  ...
  //       <Text style={styles.lightText}>Name: {item.name}</Text>
  //     </TouchableOpacity>
  //   );
  // };
